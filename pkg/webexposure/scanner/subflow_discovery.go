...
// runMultiDomainDiscoverySubflow executes domain discovery for multiple domains and aggregates results
// Cache: results/{primaryDomain}/domain-discovery-result.json (aggregated)
// Dependencies: None (Level 0 subflow)
func (s *scanner) runMultiDomainDiscoverySubflow(domains []string, primaryDomain string, force bool, keywords []string, skipPassive bool, skipCertificate bool) (map[string]*domainscan.DomainEntry, error) {
	log := logger.GetLogger()
	resultsDir := filepath.Join("results", primaryDomain)
	cacheFile := filepath.Join(resultsDir, "domain-discovery-result.json")
	...
	// Step 3: Execute discovery for ALL domains at once
	log.Info().Msgf("Running fresh domain discovery for %d domains", len(domains))
	...
	log.Info().Msgf("Discovered %d live targets from %d seed domains", len(discoveredDomains), len(domains))

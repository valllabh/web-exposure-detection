...
// PlaywrightWebKitPDFGenerator implements common.PDFGenerator using playwright WebKit with screenshot-to-PDF
type PlaywrightWebKitPDFGenerator struct{}

// NewPlaywrightWebKitPDFGenerator creates a new playwright WebKit-based PDF generator
func NewPlaywrightWebKitPDFGenerator() common.PDFGenerator {
	return &PlaywrightWebKitPDFGenerator{}
}

// GeneratePDF generates a PDF from HTML using playwright WebKit and screenshot conversion
func (g *PlaywrightWebKitPDFGenerator) GeneratePDF(htmlPath, pdfPath string) error {
	logger := logger.GetLogger()
	logger.Debug().Msg("Initializing Playwright WebKit for PDF generation via screenshot")
	...
	logger.Debug().Msg("Launching WebKit browser")
	browser, err := pw.WebKit.Launch(playwright.BrowserTypeLaunchOptions{
		Headless: playwright.Bool(true),
	})
	if err != nil {
		logger.Error().Msgf("Failed to launch WebKit: %v", err)
